...
export function MobileNav({ solid, navItems, pathname: pathnameProp }: Props) {
  ...
  return (
    <>
      ...
      {open ? (
        /*
         * Single fixed element — no inner absolute child.
         * `fixed inset-0` anchors to the viewport regardless of any
         * positioned ancestor (SiteHeader, transformed wrappers, etc.).
         * `min-h-[100dvh]` covers the area correctly on Android Chrome /
         * Samsung Browser when browser UI is visible or hidden.
         * `w-screen` prevents any max-width container from constraining width.
        ...
        <div
          ...
          className="fixed inset-0 z-[100] flex w-screen min-h-[100dvh] flex-col lg:hidden"
          style={{
        ...
        >
          ...
          {/* Navigation links — only this region scrolls */}
          <nav
